...
- `Sources/GastosSMS/Achievements/AchievementsEngine.swift` — computes achievements **entirely on the fly** from `Transaction` data, no "unlocked" record persisted anywhere (simpler, can't drift out of sync — tradeoff: no persisted "first unlocked at" moment, so no celebration animation is possible without adding that later, see the analysis). `evaluate(transactions:)` walks every calendar month from the earliest transaction through the current month inclusive (including empty months — a quiet month after a big-spend one still counts as a save), comparing each to `SpendingTrend` against the month before it.
- `Sources/GastosSMS/Views/AchievementsView.swift` — points total, badge list (locked/unlocked per `AchievementCatalog.tiers`), and full monthly history, all derived from `AchievementsEngine.evaluate`. Shows `ContentUnavailableView` (not fake data) when there are no transactions at all yet.
